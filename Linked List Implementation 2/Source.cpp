...
Description: This program is a simple linked list program, and it will show how to remove the number inthe linked list 
...
int main()
{
	...
	cout << "Next step is to demonstrate removing the number according to its position (** the first position start with 0**)" << endl;

	while (Remove == 'Y')
	{
		cout << "Please input the position that you would like to remove (**The first position start with 0**): ";
		cin >> Pos;
		LL1.remove(Pos);

		cout << "This is the linked list after removing the position " << Pos << " :" << endl;
		LL1.PrintList();

		cout << "Would you like to remove more number in this current linked list?" << endl;
		cout << "If yes, please type 'Y'. Otherwise type any character: ";
		cin >> Remove;
